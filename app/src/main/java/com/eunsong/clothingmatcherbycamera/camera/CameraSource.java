...
public class CameraSource {
  ...
  private Size initialPreviewSize;

  public CameraSource(Activity activity, GraphicOverlay overlay, CameraCallback cameraCallback) {
    ...
    this.initialPreviewSize = new Size(metrics.widthPixels, metrics.widthPixels);
  }

  // ==============================================================================================
  // Public
  // ==============================================================================================
  public Size getInitialPreviewSize() {
    return initialPreviewSize;
  }

  public void setInitialPreviewSize(Size initialPreviewSize) {
    this.initialPreviewSize = initialPreviewSize;
    Log.v(TAG, "initialPreviewSize "+initialPreviewSize);
  }
...
//    if (sizePair == null) {
    DisplayMetrics metrics = new DisplayMetrics();
    activity.getWindowManager().getDefaultDisplay().getMetrics(metrics);
    Log.v(TAG, "w/h %s %s"+metrics.widthPixels+" / " + metrics.heightPixels);
    Log.v(TAG, "initialPreviewSize"+initialPreviewSize);

    SizePair sizePair =
          selectSizePair(
              camera,
                  /*initialPreviewSize.getWidth(),
                  initialPreviewSize.getHeight()*/
              DEFAULT_REQUESTED_CAMERA_PREVIEW_WIDTH,
              DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT);
//    }
    Log.v(TAG, "selectedPair "+sizePair.preview + " / "+sizePair.picture);
...
//    // closest aspect ratio vs. using the closest pixel area.
//    SizePair selectedPair = null;
//    ArrayList<SizePair> bigEnough = new ArrayList<>();
//    ArrayList<SizePair> notBigEnough = new ArrayList<>();
//    ArrayList<SizePair> remainds = new ArrayList<>();
//
//    int minDiff = Integer.MAX_VALUE;
//    int maxWidth = 1300;
//    for (SizePair sizePair : validPreviewSizes) {
...
//          minDiff = diff;
//
//          if (size.getHeight() == size.getWidth() * desiredHeight / desiredWidth) {
//            if (size.getWidth() >= desiredWidth && size.getHeight() >= desiredHeight) {
//              bigEnough.add(sizePair);
//            } else {
//              notBigEnough.add(sizePair);
//            }
//
//          } else {
//            remainds.add(sizePair);
//          }
////        }
//      }
//
...
//    } else if(remainds.size() > 0){
//      return Collections.max(remainds, new CompareSizesByArea());
//    } else {
//      return selectedPair;
//    }
//
...
//        float previewAspectRatio = (float) previewSize.width / (float) previewSize.height;
//        if (Math.abs(previewAspectRatio - 4/3) < ASPECT_RATIO_TOLERANCE) {
        ...
        try {
          synchronized (processorLock) {
            ...
            frameProcessor.process(
                data,
                new FrameMetadata.Companion.Builder()
