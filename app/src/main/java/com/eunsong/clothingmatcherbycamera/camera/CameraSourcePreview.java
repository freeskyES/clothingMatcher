...
//      this.setLayoutParams(new ConstraintLayout.LayoutParams(cameraSource.getPreviewSize().getWidth(), cameraSource.getPreviewSize().getHeight()));
      requestLayout();
      changeExposure();
...
//      setupPreview(width, height);
      changeExposure();
      cameraSource.onFinishPreview();
    }
  }

  @SuppressLint("MissingPermission")
  private void setupPreview(int width, int height) {
    Log.v(TAG, "cameraSource.getPreviewSize()" + cameraSource.getPreviewSize());
    Log.v(TAG, "surface w/h" + width + " / "+height);
    if (initSize == null) {
      try {
        initSize = new Size(width,height);
        Log.v(TAG, "initSize" + initSize);
        cameraSource.setInitialPreviewSize(initSize);
        cameraSource.stop();
        cameraSource.start();
      } catch (IOException e) {
        e.printStackTrace();
  ...
  private void changeExposure() {
    if (cameraSource == null || cameraSource.camera == null) return;
